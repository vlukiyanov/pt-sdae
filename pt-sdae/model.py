        ...
        for index, batch in enumerate(data_iterator):
            ...
            if update_freq is not None and index % update_freq == 0:
                ...
                if validation_loader is not None:
                    validation_output = predict(dataset, autoencoder, batch_size, cuda=cuda)
                    validation_inputs = []
                    ...
                    validation_loss_value = float(validation_loss[0])
...
def predict(
    ...
    :param model: instance of an autoencoder to train
    ...
    for index, batch in enumerate(data_iterator):
        ...
        if cuda:
            batch = batch.cuda(async=True)
        batch = batch.squeeze(1).view(batch.size(0), -1)
        features.append(model.encode(batch).detach().cpu())  # move to the CPU to prevent out of memory on the GPU
